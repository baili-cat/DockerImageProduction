...
#Mysql版本对应的列表
MONGODB_VERSION_ENUM=../conf/MONGODB_VERSION_ENUM.conf
#Mysql监控exporter初始化sql文件
MONGODBEXPORTINITSQL=../initdb/exporter-init.sql
...
#Oracel的mysql镜像版本
MONGODBDOCKERTAG=`getValues_accordKey ${MONGODB_VERSION_ENUM} mongodb ${VERSION}`
#兼容mysql版本不存在的情况，版本不存在默认使用最新版本
if [ -z "$NEO4JDOCKERTAG" ]; then
  VERSION="latest"
  MONGODBDOCKERTAG="latest"
fi
...
#使用基础构建脚本，构建镜像
sh ./base-build.sh "$IMAGENAME" "$VERSION" "$MONGODBDOCKERTAG" "$DOCKERFILEPATH$DOCKERFILE" "$ARCH" "$IMAGEPATH" "$MONGODBEXPORTINITSQL"
